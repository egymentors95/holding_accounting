...
class AgedPartnerBalanceReport(models.AbstractModel):
    ...
    def _initialize_account(self, ag_pb_data, acc_id):
        ...
        ag_pb_data[acc_id]["last_payment"] = 0.0
        ag_pb_data[acc_id]["last_payment_date"] = ''
        ag_pb_data[acc_id]["last_invoice_date"] = ''
        ag_pb_data[acc_id]["last_invoice_amount"] = 0.0
        ag_pb_data[acc_id]["total_arrears"] = 0.0
    ...
    @api.model
    def _calculate_extra_info(self, ag_pb_data, acc_id, prt_id,date_at_object):
        payments = self.env['account.payment'].sudo()
        invoices = self.env['account.move'].sudo()
        pay = payments.search([('partner_id','=',prt_id),('state','not in',['draft','cancelled'])],limit=1)
        invoice = invoices.search([('partner_id','=',prt_id)],limit=1)

        ag_pb_data[acc_id]["last_invoice_date"] = invoice and invoice.invoice_date or ''
        ag_pb_data[acc_id][prt_id]["last_invoice_date"] = invoice and invoice.invoice_date or ''
        ag_pb_data[acc_id]["last_payment_date"] = pay and pay.date or ''
        ag_pb_data[acc_id][prt_id]["last_payment_date"] = pay and pay.date or ''

        ag_pb_data[acc_id]["last_invoice_amount"] = invoice and invoice.amount_total or 0.0
        ag_pb_data[acc_id][prt_id]["last_invoice_amount"] = invoice and invoice.amount_total or 0.0
        ag_pb_data[acc_id]["last_payment"] = pay and pay.amount or 0.0
        ag_pb_data[acc_id][prt_id]["last_paymemt"] = pay and pay.amount or 0.0
    ...
    def _get_move_lines_data(
        ...
        for move_line in move_lines:
            ...
            ag_pb_data = self._calculate_extra_info(
                    ag_pb_data,
                    acc_id,
                    prt_id,
                    date_at_object,
        ...
        print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>ag_pb_data",ag_pb_data)
        print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>accounts_data",accounts_data)
        print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>partners_data",partners_data)
        print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>journals_data",journals_data)
            ...
            account.update(
                {
                    ...
                    "last_payment": ag_pb_data[acc_id]["last_payment"],
                    "last_payment_date": ag_pb_data[acc_id]["last_payment_date"] ,
                    "last_invoice_date": ag_pb_data[acc_id]["last_invoice_date"] ,
                    "last_invoice_amount" : ag_pb_data[acc_id]["last_invoice_amount"] ,
                    "total_arrears" : ag_pb_data[acc_id]["total_arrears"] ,
    ...
    def _get_report_values(self, docids, data):
        ...
        date_at_object = datetime.strptime(str(date_at), "%Y-%m-%d").date()
        ...
        print("--------------------------aged_partner_data",aged_partner_data)
        return {
            "doc_ids": [wizard_id],
