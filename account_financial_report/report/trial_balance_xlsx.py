...
class TrialBalanceXslx(models.AbstractModel):
    ...
    def _get_report_columns(self, report):
        if report.debit_credit and not report.show_partner_details:
            res = {
                ...
                6: {
                    "header": _("Ending Debit"),
                    "field": "ending_debit",
                    "type": "ending",
                    "width": 14,
                },
                7: {
                    "header": _("Ending Credit"),
                    "field": "ending_credit",
                    "type": "amount",
                    "width": 14,
                },
            ...
            return res
        elif not report.debit_credit and not report.show_partner_details:
            res = {
                0: {"header": _("Code"), "field": "code", "width": 10},
                1: {"header": _("Account"), "field": "name", "width": 60},
                2: {
            ...
            if report.foreign_currency:
                ...
                res = {**res, **foreign_currency}
            return res
        elif report.debit_credit and report.show_partner_details:
            res = {
    ...
    def _generate_report_content(self, workbook, report, data, report_data):
        ...
        debit_credit = res_data["debit_credit"]
        ...
        # For each account
        if not show_partner_details:
            total_line = {'total': True, 'initial_balance': [0, 0, 0], 'credit': 0, 'debit': 0, 'balance': 0,
                          'ending_balance': [0, 0, 0]}
            for balance in trial_balance:
                total_line['initial_balance'][0] += balance['initial_balance'][0]
                total_line['initial_balance'][1] += balance['initial_balance'][1]
                total_line['initial_balance'][2] += balance['initial_balance'][2]
                total_line['credit'] += balance['credit']
                total_line['debit'] += balance['debit']
                total_line['balance'] += balance['balance']
                total_line['ending_balance'][0] += balance['ending_balance'][0]
                total_line['ending_balance'][1] += balance['ending_balance'][1]
                total_line['ending_balance'][2] += balance['ending_balance'][2]
                ...
                if hierarchy_on == "relation":
                    ...
                    self.write_line_from_dict(balance, report_data)
            print('ssssss', total_line)
            print('ssssss', report_data)
            ...
            for account_id in total_amount:
                ...
                accounts_data[account_id].update(
                    {
                        "initial_debit": total_amount[account_id]["initial_balance"][0],
                        "initial_credit": total_amount[account_id]["initial_balance"][1],
                        "initial_balance": total_amount[account_id]["initial_balance"][2],
                        "credit": total_amount[account_id]["credit"],
                        "debit": total_amount[account_id]["debit"],
                        "balance": total_amount[account_id]["balance"],
                        "ending_debit": total_amount[account_id]["ending_balance"][0],
                        "ending_credit": total_amount[account_id]["ending_balance"][1],
                        "ending_balance": total_amount[account_id]["ending_balance"][2],
                    }
                )
                if foreign_currency:
                    accounts_data[account_id].update(
