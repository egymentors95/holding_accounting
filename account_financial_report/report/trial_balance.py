...
class TrialBalanceReport(models.AbstractModel):
    ...
    def _get_pl_initial_balance(
        ...
        pl_initial_balance = [0.0, 0.0, 0.0]
        pl_initial_currency_balance = 0.0
        for initial_balance in initial_balances:
            pl_initial_balance[0] += initial_balance["debit"]
            pl_initial_balance[1] += initial_balance["credit"]
            pl_initial_balance[2] += initial_balance["balance"]
            if foreign_currency:
    ...
    def _compute_account_amount(
        ...
        for tb in tb_period_acc:
            ...
            total_amount[acc_id]["initial_balance"] = [0.0, 0.0, 0.0]
            # total_amount[acc_id]["ending_balance"] = tb["balance"]
            debit_credit_value = tb["debit"] - tb["credit"]
            total_amount[acc_id]["ending_balance"] = [debit_credit_value if debit_credit_value > 0 else 0,
                                                      abs(debit_credit_value if debit_credit_value < 0 else 0),
                                                      abs(tb["balance"])]
            if foreign_currency:
        ...
        for tb in tb_initial_acc:
            ...
            if acc_id not in total_amount.keys():
                total_amount[acc_id] = {}
                total_amount[acc_id]["initial_balance"] = [tb["debit"], tb["credit"], tb["balance"]]
                debit_credit_value = tb["debit"] - tb["credit"]
                total_amount[acc_id]["ending_balance"] = [debit_credit_value if debit_credit_value > 0 else 0,
                                                          abs(debit_credit_value if debit_credit_value < 0 else 0),
                                                          abs(tb["balance"])]
                ...
                # total_amount[acc_id]["ending_balance"] += tb["balance"]
                debit_credit_value = tb["debit"] - tb["credit"]
                total_amount[acc_id]["ending_balance"][0] += debit_credit_value if debit_credit_value > 0 else 0
                total_amount[acc_id]["ending_balance"][1] += abs(debit_credit_value if debit_credit_value < 0 else 0)
                total_amount[acc_id]["ending_balance"][2] += abs(tb["balance"])
                if foreign_currency:
                    total_amount[acc_id]["initial_currency_balance"] = round(
                        tb["amount_currency"], 2
                    )
    ...
    def _compute_acc_prt_amount(
        ...
        total_amount[acc_id][prt_id]["initial_balance"] = [tb["debit"], tb["credit"], tb["balance"]]
        debit_credit_value = tb["debit"] - tb["credit"]
        total_amount[acc_id][prt_id]["ending_balance"] = [debit_credit_value if debit_credit_value > 0 else 0,
                                                          abs(debit_credit_value if debit_credit_value < 0 else 0),
                                                          abs(tb["balance"])]
    ...
    def _compute_partner_amount(
        ...
        for tb in tb_period_prt:
            ...
            if tb["partner_id"]:
                ...
                total_amount[acc_id][prt_id]["initial_balance"] = [0.0, 0.0, 0.0]
        ...
        for tb in tb_initial_prt:
            ...
            if tb["partner_id"]:
                ...
                if acc_id not in total_amount.keys():
                    ...
                    total_amount[acc_id][prt_id]["initial_balance"][0] += tb["debit"]
                    total_amount[acc_id][prt_id]["initial_balance"][1] += tb["credit"]
                    total_amount[acc_id][prt_id]["initial_balance"][2] += tb["balance"]
                    # total_amount[acc_id][prt_id]["initial_balance"] += tb["balance"]
                    # total_amount[acc_id][prt_id]["ending_balance"] += tb["balance"]
                    debit_credit_value = tb["debit"] - tb["credit"]
                    total_amount[acc_id][prt_id]["ending_balance"][
                        0] += debit_credit_value if debit_credit_value > 0 else 0
                    total_amount[acc_id][prt_id]["ending_balance"][
                        1] += abs(debit_credit_value if debit_credit_value < 0 else 0)
                    total_amount[acc_id][prt_id]["ending_balance"][2] += abs(tb["balance"])
                    if foreign_currency:
                        total_amount[acc_id][prt_id][
    ...
    def _get_data(
        ...
        for account in accounts:
            tb_initial_acc.append(
                {"account_id": account.id, "debit": 0.0, "credit": 0.0, "balance": 0.0, "amount_currency": 0.0}
            )
        ...
        if hide_account_at_0:
            if debit_credit:
                tb_initial_acc = [p for p in tb_initial_acc if p["debit"] != 0 or p["credit"] != 0]
            else:
        ...
        if show_partner_details:
            tb_initial_prt_bs = self.env["account.move.line"].read_group(
                domain=initial_domain_bs,
                fields=["account_id", "partner_id", "debit", "credit", "balance", "amount_currency"],
            ...
            if debit_credit:
                tb_initial_prt = [p for p in tb_initial_prt if p["debit"] != 0 or p["credit"] != 0]
            else:
        ...
        if unaffected_id:
            if unaffected_id not in accounts_ids:
                ...
                total_amount[unaffected_id]["initial_balance"] = [0.0, 0.0, 0.0]
                ...
                total_amount[unaffected_id]["ending_balance"] = [0.0, 0.0, 0.0]
                if foreign_currency:
        ...
        if unaffected_id:
            # total_amount[unaffected_id]["ending_balance"] += pl_initial_balance[2]
            debit_credit_value = pl_initial_balance[0] - pl_initial_balance[1]
            total_amount[unaffected_id]["ending_balance"][0] += debit_credit_value if debit_credit_value > 0 else 0
            total_amount[unaffected_id]["ending_balance"][1] += abs(debit_credit_value if debit_credit_value < 0 else 0)
            total_amount[unaffected_id]["ending_balance"][2] += abs(pl_initial_balance[2])
            # total_amount[unaffected_id]["initial_balance"] += pl_initial_balance
            total_amount[unaffected_id]["initial_balance"][0] += pl_initial_balance[0]
            total_amount[unaffected_id]["initial_balance"][1] += pl_initial_balance[1]
            total_amount[unaffected_id]["initial_balance"][2] += pl_initial_balance[2]
            if foreign_currency:
                total_amount[unaffected_id][
                    "ending_currency_balance"
                ] += pl_initial_currency_balance
        ...
        for group_id in group_ids:
            if groups_data[group_id]["parent_id"]:
                ...
                if nw_id in groups_data.keys():
                    ...
                    groups_data[nw_id]["initial_balance"][0] += groups_data[group_id]["initial_balance"][0]
                    groups_data[nw_id]["initial_balance"][1] += groups_data[group_id]["initial_balance"][1]
                    groups_data[nw_id]["initial_balance"][2] += groups_data[group_id]["initial_balance"][2]
                    ...
                    debit_credit_value = groups_data[group_id]["ending_balance"][0] - \
                                         groups_data[group_id]["ending_balance"][1]
                    groups_data[nw_id]["ending_balance"][0] += debit_credit_value if debit_credit_value > 0 else 0
                    groups_data[nw_id]["ending_balance"][1] += abs(debit_credit_value if debit_credit_value < 0 else 0)
                    groups_data[nw_id]["ending_balance"][2] += abs(groups_data[group_id]["ending_balance"][2])
    ...
    def _get_groups_data(self, accounts_data, total_amount, foreign_currency):
            ...
            groups_data.update(
                {
                    group.id: {
                        ...
                        "complete_code": group.complete_code,
                        "account_ids": group.compute_account_ids.ids,
                        "initial_balance": [0.0, 0.0, 0.0],
                        "credit": 0.0,
                        "debit": 0.0,
                        "balance": 0.0,
                        "ending_balance": [0.0, 0.0, 0.0],
                    }
                }
            )
            if foreign_currency:
        ...
        for group_id in account_group_relation.keys():
            for account_id in account_group_relation[group_id]:
                ...
                groups_data[group_id]["initial_balance"][0] += total_amount[account_id]["initial_balance"][0]
                groups_data[group_id]["initial_balance"][1] += total_amount[account_id]["initial_balance"][1]
                groups_data[group_id]["initial_balance"][2] += total_amount[account_id]["initial_balance"][2]
                ...
                debit_credit_value = total_amount[account_id]["ending_balance"][0] - \
                                     total_amount[account_id]["ending_balance"][1]
                groups_data[group_id]["ending_balance"][0] += debit_credit_value if debit_credit_value > 0 else 0
                groups_data[group_id]["ending_balance"][1] += abs(debit_credit_value if debit_credit_value < 0 else 0)
                groups_data[group_id]["ending_balance"][2] += abs(total_amount[account_id]["ending_balance"][2])
                if foreign_currency:
        ...
        for group in groups:
            ...
            for account in accounts_data.values():
                if group.code_prefix_start == account["code"][:len_group_code]:
                    ...
                    groups_data[group_id]["initial_balance"][0] += total_amount[acc_id]["initial_balance"][0]
                    groups_data[group_id]["initial_balance"][1] += total_amount[acc_id]["initial_balance"][1]
                    groups_data[group_id]["initial_balance"][2] += total_amount[acc_id]["initial_balance"][2]
                    ...
                    debit_credit_value = total_amount[acc_id]["ending_balance"][0] - \
                                         total_amount[acc_id]["ending_balance"][1]
                    groups_data[group_id]["ending_balance"][0] += debit_credit_value if debit_credit_value > 0 else 0
                    groups_data[group_id]["ending_balance"][1] += abs(
                        debit_credit_value if debit_credit_value < 0 else 0)
                    groups_data[group_id]["ending_balance"][2] += abs(total_amount[acc_id]["ending_balance"][2])
                    if foreign_currency:
