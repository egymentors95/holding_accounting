...
from odoo.addons.send_report_by_email.send_report_by_email import action_send_report
...
class GeneralLedgerReportWizard(models.TransientModel):
    ...
    parent_account_ids = fields.Many2many(comodel_name="account.account", relation="ledger_parent_account_rel",
                                          column1="ledger_id", column2="account_id",
                                          string="Filter Parent accounts", )
    ...
    parent_analytic_ids = fields.Many2many(comodel_name="account.analytic.account",
                                           relation="ledger_parent_analytic_rel",
                                           column1="ledger_id", column2="analytic_id",
                                           string="Filter Parent analytic accounts", )
    ...
    display_options = fields.Selection(
        [('one_table', 'One Table'),
         ('group_by_account', 'Group By Account'),
         ('account_per_page', 'Account Per Page')]
    )

    @api.onchange('parent_account_ids')
    def onchange_parent_account_ids(self):
        if self.parent_account_ids:
            self.account_ids = self.parent_account_ids.mapped('child_ids')

    @api.onchange('parent_analytic_ids')
    def onchange_parent_analytic_ids(self):
        if self.parent_analytic_ids:
            self.cost_center_ids = self.parent_analytic_ids.mapped('child_ids')
    ...
    def button_send_email(self):
        data = self._prepare_report_general_ledger()
        report_id = "account_financial_report.general_ledger"
        title = 'General Ledger Report'
        return action_send_report(self, report_id, title, data)
