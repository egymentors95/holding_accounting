...
class AccountJournal(models.Model):
    ...
    def _l10n_sa_get_production_CSID(self, OTP=None):
        ...
        renew = False
        zatca_format = self.env.ref('exp_l10n_sa_edi.edi_sa_zatca')
    ...
    def _l10n_sa_get_compliance_files(self):
        ...
        for file in file_names:
            fpath = get_module_resource('exp_l10n_sa_edi', 'tests/compliance', file)
    ...
    def _l10n_sa_run_compliance_checks(self):
        ...
        for fname, fval in compliance_files.items():
            invoice_hash_hex = self.env['account.edi.xml.ubl_21.zatca']._l10n_sa_generate_invoice_xml_hash(
                fval).decode()
            digital_signature = self.env.ref('exp_l10n_sa_edi.edi_sa_zatca')._l10n_sa_get_digital_signature(self.company_id, invoice_hash_hex).decode()
    ...
    def _l10n_sa_prepare_compliance_xml(self, xml_name, xml_raw, PCSID, signature):
        ...
        signed_xml = self.env.ref('exp_l10n_sa_edi.edi_sa_zatca')._l10n_sa_sign_xml(xml_content, PCSID, signature)
    ...
    def _l10n_sa_prepare_invoice_xml(self, xml_content):
        ...
        """
        ubl_extensions = etree.fromstring(self.env.ref('exp_l10n_sa_edi.export_sa_zatca_ubl_extensions')._render())
    ...
    def _l10n_sa_edi_get_next_chain_index(self):
        ...
        if not self.l10n_sa_chain_sequence_id:
            self.l10n_sa_chain_sequence_id = self.env['ir.sequence'].create({
                ...
                'code': f'exp_l10n_sa_edi.account.move.{self.id}',
    ...
    def _l10n_sa_api_get_pcsid(self):
        ...
        if not self.l10n_sa_production_csid_json:
            raise UserError(_("Please, make a request to obtain the Compliance CSID and Production CSID before sending "
                            "documents to ZATCA"))
        pcsid_validity = self.env.ref('exp_l10n_sa_edi.edi_sa_zatca')._l10n_sa_get_zatca_datetime(self.l10n_sa_production_csid_validity)
        time_now = self.env.ref('exp_l10n_sa_edi.edi_sa_zatca')._l10n_sa_get_zatca_datetime(datetime.now())
