...
class AccountAsset(models.Model):
    ...
    method = fields.Selection([('linear', 'Straight Line'), ('degressive', 'Declining'),
                               ...
                               ('product_quantity', 'علي حسب الكمية')], string='Method',
    ...
    product_qty = fields.Float("كمية المنتج")
    product_price = fields.Float("سعر الوحدة")
    total_price = fields.Float("الاجمالي", compute="_compute_total_price", store=True, readonly=False)

    @api.depends('product_qty', 'product_price')
    def _compute_total_price(self):
        for rec in self:
            if rec.product_price and rec.product_qty:
